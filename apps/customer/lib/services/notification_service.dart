...
class NotificationService {
  ...
  /// Tracks the currently-open chat room so we don't push a duplicate
  /// conversation screen if the user is already there when the push arrives.
  static String? activeChatRoomId;

  static const _channelId = 'barberhero_bookings';
  static const _channelName = 'Booking Updates';
  static const _channelDesc = 'Notifications for booking status changes';
    ...
    switch (type) {
      case 'chat_message':
        ...
        if (roomId != null) {
          ...
          // Fire-and-forget — navigates after resolving the room.
          unawaited(_openChatConversation(roomId));
          return;
        }
        // Missing chatRoomId — fall back to Chat tab (index 2).
        nav.pushNamedAndRemoveUntil('/home', (_) => false, arguments: 2);
        return;
      case 'booking_status':
      case 'booking_request':
        nav.pushNamedAndRemoveUntil('/home', (_) => false, arguments: 1);
        return;
      default:
  ...
  static Future<void> _openChatConversation(String roomId) async {
    ...
    // Need the user's own id to decide which bubble is theirs, plus the
    // peer's profile for the screen header. Pulled from /chat/rooms.
    try {
      final rooms = await api.getChatRooms();
      final match = rooms.where((r) => r.id == roomId).toList();
      if (match.isEmpty) {
        // Room no longer visible — fall back to the inbox.
        nav.pushNamedAndRemoveUntil('/home', (_) => false, arguments: 2);
        return;
      }
      ...
      // Land them on the inbox first, then push the conversation on top —
      // so Back returns to the list, not to whatever they were doing.
      nav.pushNamedAndRemoveUntil('/home', (_) => false, arguments: 2);
      nav.push(MaterialPageRoute(
        builder: (_) => ChatConversationScreen(
          roomId: room.id,
          peerName: room.barber?.fullName ?? 'Barber',
          peerPhoto: room.barber?.profilePhoto,
          currentUserId: currentUserId,
        ),
      ));
    } catch (_) {
      // Resolution failed — at least land on the inbox.
      nav.pushNamedAndRemoveUntil('/home', (_) => false, arguments: 2);
    }
  }
}
