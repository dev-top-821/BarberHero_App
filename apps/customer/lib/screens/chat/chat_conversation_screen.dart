...
// unawaited is in dart:async — no extra import needed.
...
/// One-on-one conversation. Streams messages live from Firestore (mirrored
/// from the backend Postgres truth). Older messages outside the Firestore
/// cache window are paginated from the backend on demand. Sends still go
...
class _ChatConversationScreenState extends State<ChatConversationScreen>
  ...
  // Older messages fetched from the Postgres history endpoint, kept separate
  // from the live Firestore stream so we can prepend without thrashing the
  // snapshot's identity.
  final List<ChatMessage> _olderHistory = [];
  StreamSubscription<List<ChatMessage>>? _msgSub;
  StreamSubscription<ChatRoomMeta?>? _metaSub;

  // REST-fallback state (used only when Firestore real-time is unavailable
  // — e.g. backend deploy without firebase-token endpoint, Admin SDK
  // unconfigured, Firestore not provisioned). One-shot fetch + 15s polling.
  Timer? _pollTimer;
  ...
  void initState() {
    ...
    // Marker so incoming FCM chat pushes for *this* room don't deep-link
    // us back onto ourselves.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // Pause REST polling when backgrounded — FCM pushes still arrive.
  ...
  Future<void> _bootstrap() async {
    ...
    if (!firestoreOk) {
      // Real-time backend isn't available — fall back to REST polling so
      // chat still works (just with ~15s latency instead of instant).
      _useRestFallback = true;
      await _restLoadInitial();
      return;
    }
    // Fire-and-forget read stamp on open; the Firestore meta listener will
    // also pick up the resulting unread reset.
    ...
    _msgSub = realtime.messagesStream(widget.roomId).listen(
      (messages) {
        ...
        // Mark new peer messages as read while the screen is open.
  ...
  // ─── REST fallback path ────────────────────────────────────
  // Used only when ensureSignedIn() reports Firestore unavailable. Mirrors
  // the original polling behavior we had before the Firestore migration.
  ...
  void _onScroll() {
    // ListView is reverse:false, so the *top* is pixels == 0. Older messages
    // get fetched when the user scrolls near the top.
  ...
  /// Combined ascending list: older history (Postgres) + live (Firestore).
  /// Dedupes on id in case the same message appears in both windows.
  ...
  Future<void> _send() async {
    ...
    try {
      final api = context.read<ApiClient>();
      // Backend persists to Postgres + mirrors to Firestore — the Firestore
      // listener will deliver the new message back to us shortly.
            ...
            child: _loading
                ...
                : _loadError != null
                    ...
                    : messages.isEmpty
                        ...
                        : Builder(builder: (_) {
                            // Find the index of the last message I sent
                            // that the peer has already read. That one
                            // gets the "Seen" label.
                            int seenIdx = -1;
